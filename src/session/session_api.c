...
#ifdef HAVE_DIAGNOSTIC
    // FIXME - destroying thread_check->lock while lock is still held!
    // The lock is only freed when a thread has exited all API calls, but conn_close is an API call
  ...
  WT_CURSOR **cursorp)
{
    ...
    WT_ASSERT(session,
      strcmp(uri, WT_HS_URI) == 0 || session->hs_cursor_counter == 0 ||
        ...
        (S2BT_SAFE(session) != NULL && F_ISSET_ATOMIC_32(S2BT(session), WT_BTREE_VERIFY)));
