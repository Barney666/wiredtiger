...
#define WT_WITH_TXN_ISOLATION(s, iso, op)                                                         \
    do {                                                                                          \
        ...
        WT_ASSERT((s),                                                                            \
          txn_shared->id ==                                                                       \
              saved_txn_shared                                                                    \
                .id && /* // HERE - A lot of atomics required here. This is a good example of the \
                          worst-case cosmetic impacts of the TSan changes. In particular the      \
                          __wt_atomic_storev64(&foo, __wt_atomic_loadv64(&bar)) cases */          \
            (__wt_atomic_loadv64(&txn_shared->metadata_pinned) ==                                 \
                __wt_atomic_loadv64(&saved_txn_shared.metadata_pinned) ||                         \
              __wt_atomic_loadv64(&saved_txn_shared.metadata_pinned) == WT_TXN_NONE) &&           \
            (__wt_atomic_loadv64(&txn_shared->pinned_id) ==                                       \
                __wt_atomic_loadv64(&saved_txn_shared.pinned_id) ||                               \
              __wt_atomic_loadv64(&saved_txn_shared.pinned_id) == WT_TXN_NONE));                  \
        __wt_atomic_storev64(                                                                     \
          &txn_shared->metadata_pinned, __wt_atomic_loadv64(&saved_txn_shared.metadata_pinned));  \
        __wt_atomic_storev64(                                                                     \
          &txn_shared->pinned_id, __wt_atomic_loadv64(&saved_txn_shared.pinned_id));              \
    } while (0)
